...
import {
  createMultipleClassOfferings,
  deleteClassOffering,
  setClassOfferingStatus,
...
const sectionMatchesCourse = (
  ...
  return sectionCourseValues.some(
    (sectionValue) =>
      selectedCourseValues.includes(sectionValue)
  );
};
...
function ClassAssignmentManagement() {
  ...
  */
  ...
  | IMPORTANT:
  |
  | Duplicate checking happens only inside the exact:
  |
  | section
  | school year
  | semester
  ...
  */
  ...
  const handleOfficialClassSelect = (
    section
  ) => {
    if (
      !section?.id ||
  ...
  */
  ...
  const toggleSubject = (
    ...
    setFormData(
      (previous) => {
        const selected =
          previous.selected_subject_ids.includes(
            subjectId
          );
  ...
  const handleSelectAllSubjects =
    () => {
      setFormData(
        (previous) => {
          const allSelected =
            selectableSubjectIds.length >
              0 &&
            selectableSubjectIds.every(
              (subjectId) =>
                previous.selected_subject_ids.includes(
                  subjectId
                )
  ...
  */
  ...
  const validateForm = () => {
    ...
    if (
      formData.selected_subject_ids.some(
        (id) =>
          existingSubjectIds.includes(
            id
          )
  ...
  */
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      |--------------------------------------------------------------------------
      |
      | Each subject becomes its own class_offering row.
      |
      | Example:
      |
      | IT301 + BSIT3A + Teacher A
      |
      | IT301 + BSIT3B + Teacher B
      |
      | These are two different class offerings even though
      | they use the same subject.
      |--------------------------------------------------------------------------
      */

      const assignments =
        formData.selected_subject_ids.map(
          (subjectId) => ({
            subject_id:
              subjectId,
      ...
      const createdOfferings =
        await createMultipleClassOfferings(
          {
            section_id:
              formData.section_id,
        ...
        html: `
          <div style="text-align:left;line-height:1.6">
            ...
            <p style="color:#b45309;margin-top:8px">
              Existing clearance steps will remain.
              This only affects future requests.
            </p>
                      ...
                      {selectedOfficialClass && (
                        <div className="mb-4 rounded-2xl border border-blue-200 bg-blue-50 p-4">
                          ...
                          <p className="mt-2 font-black text-slate-900">
                            ...
                            —{" "}
                            {
                              selectedOfficialClass.year_level
                            }{" "}
                            — Block{" "}
                            {
